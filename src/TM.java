...
public class TM {
   ...
   public void appMain(String[] args) throws FileNotFoundException {
	  ...
	  if(args.length != 1) {       //Creates a ArrayOutOfBounds if you don't do this because Summary() can't use args[1], but Summary(name) does.
		  data = args[1];          //Therefore data = args[1] only if you're using Summary(name), Start/Stop(name), and Describe(name, description)
  	  }
	  String cmd = args[0];
      String desc = "";
      ...
      switch(cmd) {               //To switch between the Start, Stop, Describe, and Summary commands
         ...
         case "Stop": Stop(data);
            break;
         case "Describe": Describe(data, desc);
            break;
         case "Summary": {
        	 if(args.length == 1) {
        		 Summary();          //For a summary of all tasks in Task Manager
        	 }else {
        		 Summary(data);      //For a summary of an individual task in TaskManager
        	 }
